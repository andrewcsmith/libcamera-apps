...
typedef LibcameraApp<StillOptions> LibcameraHdr;
	...
	for (unsigned int count = 0; ; count++)
	{
		...
		else if (count == PREVIEW_FRAMES)
		{
			...
			jpeg_save(app.Mmap(buffers[vf_stream]), vf_w, vf_h, vf_stride,
					  ...
					  metadata, "short.jpg", app.CameraId(), app.options);
		...
		else
		{
			...
			if (count == PREVIEW_FRAMES + HDR_CONFIG.num_frames)
			{
				...
				jpeg_save({&output[0]}, w, h, stride, stream->configuration().pixelFormat,
						  metadata, "hdr.jpg", app.CameraId(), app.options);
				return;
			}
